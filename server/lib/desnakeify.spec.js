...
describe('db/lib/desnakeify()', function desnakeifyTest() {
  it('should snakeCase all attributes of the passed object', async () => {
    expect(
      await desnakeify({ first: 1, second_item: 'second', thirdItem: [3] }),
    ).to.eql({ first: 1, secondItem: 'second', thirdItem: [3] });
  });

  it('should snakeCase all attributes of all objects in the passed array', async () => {
    expect(
      await desnakeify([
  ...
  it('should work when a promise is passed', async () => {
    expect(
      await desnakeify(Promise.resolve({ a_a: 'A', bB: 'B', c: 'C' })),
    ).to.eql({ aA: 'A', bB: 'B', c: 'C' });
  });

  it('should return the passed data when neither an array nor object is passed', async () => {
  ...
  it('should not mess up Date objects', async () => {
